package com.salesmanager.core.business.modules.cms.product.digitalocean;
 ...
 *
 */
public class DOSpaceProductContentFileManager
  ...
  private static final Logger LOGGER = LoggerFactory.getLogger(DOSpaceProductContentFileManager.class);
  ...
  public static DOSpaceProductContentFileManager getInstance() {
    ...
    if (fileManager == null) {
      fileManager = new DOSpaceProductContentFileManager();
    }
  ...
  public void removeProductImage(ProductImage productImage) throws ServiceException {
    try {
      ...
      String nodePath = nodePath(productImage.getProduct().getMerchantStore().getCode(),
              productImage.getProduct().getSku());

      s3.deleteObject(bucketName, nodePath + productImage.getProductImage());
  ...
  @Override
  public void removeProductImage(ProductImage productImage, FileContentType contentType) throws ServiceException {
    try {
      // get buckets
      String bucketName = bucketName();
      ...
      String nodePath = nodePath(productImage.getProduct().getMerchantStore().getCode(),
              productImage.getProduct().getSku(), String.valueOf(productImage.getId()), contentType);
  ...
  public void addProductImage(ProductImage productImage, ImageContentFile contentImage)
    ...
    try {
      ...
      String nodePath = this.nodePath(productImage.getProduct().getMerchantStore().getCode(),
          productImage.getProduct().getSku(), String.valueOf(productImage.getId()), contentImage);
  ...
  private String nodePath(String storeCode, String productSku, String productImageId, ImageContentFile contentImage) {

    StringBuilder sb = new StringBuilder();
    // node path
    String nodePath = nodePath(storeCode, productSku, contentImage);
    sb.append(nodePath);

    return sb.append(productImageId).append(Constants.SLASH).toString();
